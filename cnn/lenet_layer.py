...
class LeNetConvPoolLayer(object):
    ...
    def __init__(self, rng, input, filter_shape, image_shape, poolsize):
        ...
        conv_out = conv.conv2d(self.input_shuffled, self.W_shuffled, filter_shape=self.filter_shape, image_shape=self.input_shape)

        pooled_out = downsample.max_pool_2d(conv_out, poolsize, ignore_border=True)

        self.output = T.tanh(pooled_out + self.b.dimshuffle('x', 0, 'x', 'x'))
        #self.output = T.maximum(0,pooled_out + self.b.dimshuffle('x', 0, 'x', 'x')) relu
